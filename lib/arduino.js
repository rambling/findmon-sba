...
var TAG_NOTIFICATION = "[NOTIFICATION]";
// 5초마다 포트 상태 확인 및 연결
...
function connectToArduino() {
    ...
    getArduinoSerialComName(function(err, comName) {
        ...
        arduinoPort.on('open', function() {
            ...
            for(var i = 10; i >= 10; i--) {
                setTimeout(sendMessage, 100*i, i);
            }
        });
    });
}
...
function sendMessage(message, cb) {
    if (arduinoPort && typeof arduinoPort.isOpen === 'function' && arduinoPort.isOpen()) {
        arduinoPort.write(TAG_MESSAGE + message + '\n', function(err, results) {
